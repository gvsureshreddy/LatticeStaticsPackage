...
TrianglePressTempPairPotLat::TrianglePressTempPairPotLat(char *datafile)
{
   // First Size Defgrad
...
int TrianglePressTempPairPotLat::FindLatticeSpacing(int iter,double dx)
{
...
double TrianglePressTempPairPotLat::Beta(TDeriv dt)
{
   switch (dt)
   {
      case T0:
   ...
   cerr << "Error in TrianglePressTempPairPotLat::Beta" << endl;
   exit(-1);
}

double TrianglePressTempPairPotLat::Rhat(TDeriv dt)
{
   double num,den,rhat;
...
double TrianglePressTempPairPotLat::PairPotential(double r2,YDeriv dy,TDeriv dt)
{
   double beta=Beta(),
      rhat=Rhat(),
      r = sqrt(r2),
...
double TrianglePressTempPairPotLat::PI(const Vector &Dx,const Vector &DX,
				 int r, int s)
{
   return (Dx[r]*DX[s] + DX[r]*Dx[s]);
}

double TrianglePressTempPairPotLat::PSI(const Vector &DX,
...
double TrianglePressTempPairPotLat::pwr(const double &x,const unsigned y)
{
   if (y==1)
   {
      return x;
...
int TrianglePressTempPairPotLat::IND(int i,int j)
{
   if (i==j)
      return i;
   else
...
int TrianglePressTempPairPotLat::IND(int k,int l,int m,int n)
{
   if (k==l)
   {
      if (m==n)
...
Matrix TrianglePressTempPairPotLat::Phi(unsigned moduliflag,YDeriv dy,TDeriv dt)
{
   ...
   for (int p=0;p<DIM2;p++)
   {
      ...
      CurrentInfluanceDist = int(ceil(Influancedist[p])) + 1;
...
void TrianglePressTempPairPotLat::Print(ostream &out,PrintDetail flag)
{
   int W=out.width();
   ...
   {
      case PrintLong:
	 out << "TrianglePressTempPairPotLat:" << endl << endl
	 ...
	 cout << "TrianglePressTempPairPotLat:" << endl << endl
	      << "Cell Reference Length: " << setw(W) << RefLen_ << endl
	      << "Influance Distance   : " << setw(W) << InfluanceDist_ << endl
	      << "Potential Parameters : "
	      << "A0=  " << setw(W) << A0_
...
ostream &operator<<(ostream &out,TrianglePressTempPairPotLat &A)
{
   A.Print(out,Lattice::PrintShort);
   return out;
}

const double TrianglePressTempPairPotLat::Alt[DIM2][DIM2] = {0.0, 1.0, -1.0, 0.0};
