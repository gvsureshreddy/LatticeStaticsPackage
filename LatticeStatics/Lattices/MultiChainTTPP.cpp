...
const int MultiChainTTPP::DIM1 = 1;
   ...
   : Lattice(Input)
{
   ...
   if (DOFMAX < DOFS)
   {
      cerr << "Error (MultiChainTTPP()): DOFMAX < " << DOFS << " in Lattice.h" << "\n";
      exit(-5);
   }
...
double MultiChainTTPP::energy(PairPotentials::TDeriv dt)
{
   ...
   {
      cerr << "Error in MultiChainTTPP::energy" << "\n";
      exit(-1);
   }
...
Matrix MultiChainTTPP::stiffness(PairPotentials::TDeriv dt,LDeriv dl)
{
   ...
   if (dl==L0)
   {
      for (ChainSum_.Reset();!ChainSum_.Done();++ChainSum_)
      {
         ...
         {
            cerr << "Error in MultiChainTTPP::stiffness" << "\n";
            exit(-1);
         }
