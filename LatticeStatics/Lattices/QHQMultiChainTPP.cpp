...
const int QHQMultiChainTPP::DIM1 = 1;

QHQMultiChainTPP::~QHQMultiChainTPP()
{
   delete[] BodyForce_;
   delete[] SpeciesMass_;
   delete[] AtomicMass_;
   ...
   Lattice(Input, Echo)
{
   ...
   PerlInput::HashStruct Hash = Input.getHash("Lattice", "QHQMultiChainTPP");
...
int QHQMultiChainTPP::FindLatticeSpacing(int const& iter)
{
   // Lambda_=0.0;
   // NTemp_=1.0;
   Lambda_ = RefLambda_;
...
void QHQMultiChainTPP::SetParameters(double const* const Vals, int const& ResetRef)
{
...
double QHQMultiChainTPP::PI(double const* const Dx, double const* const DX) const
{
   return 2.0 * Dx[0] * DX[0];
}

double QHQMultiChainTPP::PSI(double const* const DX) const
{
   return 2.0 * DX[0] * DX[0];
}

double QHQMultiChainTPP::OMEGA(double const* const Dx, int const& p, int const& q,
                               int const& i)
const
{
   return LagrangeCB_ ?
...
double QHQMultiChainTPP::SIGMA(int const& p, int const& q, int const& i, int const& j)
const
{
   return LagrangeCB_ ?
...
double QHQMultiChainTPP::GAMMA(double const* const Dx, double const* const DX,
...
double QHQMultiChainTPP::THETA(double const* const DX, int const& p, int const& q,
                               int const& i) const
{
   return LagrangeCB_ ?
          4.0 * RefLattice_[0][0] * DELTA(i, p, q) * DX[0] :
...
double QHQMultiChainTPP::XI(int const& p, int const& q, int const& i, int const& j)
const
{
   return LagrangeCB_ ?
...
double QHQMultiChainTPP::LAMDA(int const& p, int const& q, int const& i, int const& j)
const
{
   return LagrangeCB_ ?
...
double QHQMultiChainTPP::E0() const
{
   double E0, Tsq;
...
double QHQMultiChainTPP::energy(PairPotentials::TDeriv const& dt) const
{
   double Phi = 0.0;
   double Vr;
   ...
   {
      cerr << "Error in QHQMultiChainTPP::energy" << "\n";
      exit(-1);
   }
...
double QHQMultiChainTPP::FreeEnergy(PairPotentials::TDeriv const& dt) const
{
   double FPhi = 0.0;
   double Vr;
   ...
   for (ChainIter_.Reset(); !ChainIter_.Done(); ++ChainIter_)
   {
      for (int k = 0; k < INTERNAL_ATOMS; ++k)
      {
         if ((HEigVals_static[i])[0][k] > Tol)
         {
            if (dt == PairPotentials::T0)
            {
               Term = sqrt((HEigVals_static[i])[0][k]);
            ...
            {
               Term = sqrt((HEigVals_static[i])[0][k]);
               TermT = 0.0;      // (HEigValsT_static[i])[0][k]/(2*Term);
            ...
            else if (dt == PairPotentials::D2T)
            {
               Term = sqrt((HEigVals_static[i])[0][k]);
               TermT = 0.0;       // (HEigValsT_static[i])[0][k]/(2*Term);
               TermTT = 0.0;      // ((HEigValsTT_static[i])[0][k]-2*TermT*TermT)/(2*Term);
            ...
            {
               cerr << "Error in QHQMultiChainTPP::FreeEnergy" << "\n";
               exit(-1);
            }
         }
      }
...
Vector const& QHQMultiChainTPP::E1() const
{
...
Vector const& QHQMultiChainTPP::stress(PairPotentials::TDeriv const& dt,
                                       LDeriv const& dl) const
{
   ...
   if (dl == L0)
   {
      ...
      for (SCLDChainSum_.Reset(); !SCLDChainSum_.Done(); ++SCLDChainSum_)
      {
         ...
         {
            cerr << "Error in QHQMultiChainTPP::stress" << "\n";
            exit(-1);
         }
...
Vector const& QHQMultiChainTPP::Fstress(PairPotentials::TDeriv const& dt, LDeriv const& dl) const
{
   double Vr;
         ...
         for (int k = 0; k < INTERNAL_ATOMS; ++k)
         {
            if ((HEigVals_static[i])[0][k] > Tol)
            {
               ...
               if (dt == PairPotentials::T0)
               {
                  ...
                  for (int x = 0; x < DOFS; ++x)
                  {
                     TermDOF = ((HEigValsDOF_static[x])[i])[0][k] / (2.0 * Term);
               ...
               {
                  Term = sqrt((HEigVals_static[i])[0][k]);
                  TermT = 0.0;        // (HEigValsT_static[i])[0][k]/(2.0*Term);

                  for (int x = 0; x < DOFS; ++x)
                  {
                     TermDOF = ((HEigValsDOF_static[x])[i])[0][k] / (2.0 * Term);
                     TermTDOF = 0.0;        // (((HEigValsTDOF_static[x])[i])[0][k] - 2.0*TermDOF*TermT)/(2.0*Term);
               ...
               {
                  cerr << "Error in QHQMultiChainTPP::Fstress, D2T" << "\n";
                  exit(-1);
               }
            }
         }
...
Matrix const& QHQMultiChainTPP::E2() const
{
...
Matrix const& QHQMultiChainTPP::stiffness(PairPotentials::TDeriv const& dt,
                                          LDeriv const& dl) const
{
   double phi, phi1;
   double Vr;
   ...
   if (dl == L0)
   {
      for (SCLDChainSum_.Reset(); !SCLDChainSum_.Done(); ++SCLDChainSum_)
      {
         ...
         {
            cerr << "Error in QHQMultiChainTPP::stiffness" << "\n";
            exit(-1);
         }
...
Matrix const& QHQMultiChainTPP::Fstiffness(PairPotentials::TDeriv const& dt, LDeriv const& dl) const
{
   double Vr;
            ...
            if ((HEigVals_static[i])[0][k] > Tol)
            {
               if (dt == PairPotentials::T0)
               {
                  ...
                  for (int x = 0; x < DOFS; ++x)
                  {
                     ...
                     for (int y = x; y < DOFS; ++y)
                     {
                        TermDOF2 = ((HEigValsDOF_static[y])[i])[0][k] / (2.0 * Term);
                        TermDOFDOF = ((((HEigValsDOFDOF_static[x])[y])[i])[0][k] - 2.0 * TermDOF * TermDOF2) / (2.0 * Term);
               ...
               {
                  cerr << "Error in QHQMultiChainTPP::Fstiffness, DT" << "\n";
                  exit(-1);
               }
            }
         }
   ...
   {
      cerr << "Unknown LDeriv dl in QHQMultiChainTPP::Fstiffness()" << "\n";
      exit(-1);
   }
...
Matrix const& QHQMultiChainTPP::E3() const
{
...
Matrix const& QHQMultiChainTPP::E4() const
{
...
Matrix const& QHQMultiChainTPP::CondensedModuli() const
{
   Matrix stiff = E2();
   int intrn = DOFS - 1;
...
Vector const& QHQMultiChainTPP::ThermalExpansion() const
{
...
int QHQMultiChainTPP::comp(void const* const a, void const* const b)
{
   double t;
...
int QHQMultiChainTPP::abscomp(void const* const a, void const* const b)
{
   double t;
...
void QHQMultiChainTPP::interpolate(Matrix* const EigVals, int const& zero,
...
CMatrix const& QHQMultiChainTPP::ReferenceDynamicalStiffness(Vector const& K, PairPotentials::TDeriv const& dt, int const& DOFderiv, int const& x, int const& y) const
{
   ...
   for (SCLDChainSum_.Reset(); !SCLDChainSum_.Done(); ++SCLDChainSum_)
   {
      ...
      if (SCLDChainSum_.Atom(0) != SCLDChainSum_.Atom(1))
      {
         ...
         if (DOFderiv == 0)
         {
            ...
            {
               cerr << "Error in QHQMultiChainTPP::ReferenceDynamicalStiffness - DOFderiv = 0,D3T" << "\n";
               exit(-1);
            }
         ...
         else if (DOFderiv == 1)
         {
            ...
            {
               cerr << "Error in QHQMultiChainTPP::ReferenceDynamicalStiffness - DOFderiv = 1,D2T" << "\n";
               exit(-1);
            }
         ...
         else if (DOFderiv == 2)
         {
            ...
            {
               cerr << "Error in QHQMultiChainTPP::ReferenceDynamicalStiffness - DOFderiv = 2,DT,D2T" << "\n";
               exit(-1);
            }
            ...
            {
               cerr << "Error in QHQMultiChainTPP::ReferenceDynamicalStiffness - DOFderiv = 2" << "\n";
               exit(-1);
            }
         }
         else
         {
            cerr << "Error in QHQMultiChainTPP::ReferenceDynamicalStiffness - DOFderiv = 3" << "\n";
            exit(-1);
         }
      }
      else
...
void QHQMultiChainTPP::ReferenceDispersionCurves(Vector const& K, int const& NoPTS,
...
void QHQMultiChainTPP::ReferenceHarmonic() const
{
   if (FreqCached == 0)
   {
#ifdef _OPENMP
...
void QHQMultiChainTPP::ReferenceV4() const
{
   if (FreqCached == 0)
   {
#ifdef _OPENMP
                     ...
                     if ((i * INTERNAL_ATOMS + k) >= (j * INTERNAL_ATOMS + l))
                     {
                        ...
                        for (int x = 0; x < DOFS; ++x)
                        {
                           ...
                           for (int y = x; y < DOFS; ++y)
                           {
                              ...
                              {
                                 cerr << "Error in QHQMultiChainTPP::ReferenceV4 - DOFderiv = 3" << "\n";
                                 exit(-1);
                              }
                           }
                        }
...
void QHQMultiChainTPP::ReferencePseudoHarmonic() const
{
   if (FreqCached == 0)
   {
#ifdef _OPENMP
...
int QHQMultiChainTPP::ReferenceBlochWave(Vector& K) const
{
   int i = 0;
   int NumOfNegEV = 0;
   double MinEigVal;
...
void QHQMultiChainTPP::LongWavelengthModuli(double const& dk, int const& gridsize,
                                            char const* const prefix, ostream& out)
const
{
}

void QHQMultiChainTPP::NeighborDistances(int const& cutoff, ostream& out) const
{
   Matrix NeighborDist =
...
void QHQMultiChainTPP::Print(ostream& out, PrintDetail const& flag,
...
ostream& operator<<(ostream& out, QHQMultiChainTPP& A)
{
...
void QHQMultiChainTPP::DebugMode()
{
   const char* Commands[] = {
      "INTERNAL_ATOMS",
      "DOFS",
...
void QHQMultiChainTPP::RefineEqbm(double const& Tol, int const& MaxItr,
                                  ostream* const out)
{
   Vector dx(DOFS, 0.0);
   Vector Stress = E1();
