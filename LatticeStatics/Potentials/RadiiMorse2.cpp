...
RadiiMorse2::RadiiMorse2(double A0,double B0,double Alpha,double Rref1,double Rref2,
...
double RadiiMorse2::Beta(double NTemp,TDeriv dt)
{
   double retval;
...
double RadiiMorse2::Rhat(double NTemp,TDeriv dt)
{
   double rhat;
...
double RadiiMorse2::PairPotential(double NTemp,double r2,YDeriv dy,TDeriv dt)
{
   double At=A(NTemp),
      //double At(NTemp),
      beta=Beta(NTemp),
...
void RadiiMorse2::Print(ostream &out)
{
   int W=out.width();
...
ostream &operator<<(ostream &out,RadiiMorse2 &A)
{
   A.Print(out);
   return out;
}
