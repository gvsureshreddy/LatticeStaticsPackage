...
public:
   ...
   virtual int GetNoParameters() const {return (2+LJ::GetNoParameters());}
   virtual void SetParameters(double const* const Vals);
   virtual void Print(ostream& out) const;
   virtual char const* const Type() const {return "LJSplineCutoff";}
   
   double const& CutoffStart() const {return CutoffStart_;}
   double const& CutoffEnd() const {return CutoffEnd_;}
   
   void SetCutoffStart(double const& CutoffStart) {CutoffStart_=CutoffStart;}
   void SetCutoffEnd(double const& CutoffEnd) {CutoffEnd_=CutoffEnd;}
private:
