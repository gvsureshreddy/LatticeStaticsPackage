...
public:
   ...
   virtual int GetNoParameters() const {return 12;}
   virtual void SetParameters(double const* const Vals);
   virtual void Print(ostream& out) const;
   virtual char const* const Type() const {return "GVMorse";}
   
   double const& ATPow() const {return ATPow_;}
   double const& BTPow() const {return BTPow_;}
   double const& Rtheta1Pow() const {return Rtheta1Pow_;}
   double const& Rtheta2Pow() const {return Rtheta2Pow_;}
   
   void SetATPow(double const& ATPow) {ATPow_=ATPow;}
   void SetBTPow(double const& BTPow) {BTPow_=BTPow;}
