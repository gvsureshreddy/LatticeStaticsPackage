...
public:
   ...
   ~RandomAlloy() {for (int i=0;i<BINARYPAIRBONDS;++i) delete BinaryAlloyPots[i];};
   ...
   virtual int GetNoParameters() const {return 0;}
   virtual void SetParameters(double const* const Vals) {};
   // ....
   virtual void Print(ostream& out) const;
   virtual char const* const Type() const {return "RandomAlloy";}

   void SetCompOfSubLatA(double const& CompA) {CompOfSubLatA_ = CompA;}
   void SetCompOfSubLatB(double const& CompB) {CompOfSubLatB_ = CompB;}
private:
