...
// define some transformations in an anonymous namespace
                  ...
                  const bool                   keep_boundary)
  {
    ...
    if (dim>=2)
      {
        // We do the same as in LocalGridTools::transform
        //
        ...
        for (PairIterator it2 = pairs2.begin(); it2 != pairs2.end(); ++it2)
          {
            ...
            if (LocalGridTools::orthogonal_equality(orientation,
    ...
    while ( continue_refinement && (iter<max_iterations) )
      {
        ...
        for (cell=tria.begin_active(); cell!= endc; ++cell)
          {
            std::pair<unsigned int, double> info = LocalGridTools::get_longest_direction<dim,spacedim>(cell);
