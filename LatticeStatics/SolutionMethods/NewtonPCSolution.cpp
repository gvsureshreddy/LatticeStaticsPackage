...
int NewtonPCSolution::FindNextSolution(PerlInput const& Input, int const& Width, ostream& out, ostream& pathout)
{
    int tmp = FindNextSolution(Input, Width, out);
    Restrict_->Lat()->PrintPath(out, pathout, Width);
    return tmp;
}
