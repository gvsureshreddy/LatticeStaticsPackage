...
void mexFunction(int nlhs, mxArray *plhs[ ],int nrhs, const mxArray *prhs[ ]) 
{
   ...
   for (int i=0;i<6;i++){p[i] = params[i];}                       
   p[6] = 1.0;
   p[7] = 0.0;
   Lat0.SetParameters(p);

   for (int i=6;i<12;i++){p[i-6] = params[i];}
   p[6] = 1.0;
   p[7] = 0.0;
   Lat1.SetParameters(p);

   for(int i=0;i<6;i++){q[i] = params[i];}
   q[6] = 1.0;
   q[7] = 0.0;
   for(int i=12;i<18;i++){q[i-4] = params[i];}
   q[14] = 1.0;
   q[15] = 0.0;
   for(int i=6;i<12;i++){q[i+10] = params[i];}
   q[22] = 1.0;
   q[23] = 0.0;
